...
/*   Updated: 2022/07/22 10:09:35 by jsauvain         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

int	ft_strlen_g(char *s, int n)
{
	int	i;
...
char	*strjoin(char *dst, char *src)
{
	...
	i = 0;
	j = 0;
	new = ft_calloc((ft_strlen_g(dst, 0) + ft_strlen_g(src, 0) + 1), sizeof(char));
	if (new == NULL)
		return (new);
	if (dst != NULL)
	{
...
char	*get_first_line(char *line, char *dst)
{
	...
	{
		line = ft_calloc((ft_strlen_g(dst, 1) + 1), sizeof(char));
		if (line == NULL)
			return (line);
		while (dst[j] != '\n')
			line[i++] = dst[j++];
	...
	{
		line = ft_calloc((ft_strlen_g(dst, 0) + 1), sizeof(char));
		if (line == NULL)
			return (line);
		while (dst[j])
			line[i++] = dst[j++];
...
char	*get_next_line(int fd)
{
	...
	else if (ft_strlen_g(dst[fd], 0) > 0)
	{
		line = get_first_line(line, dst[fd]);
		if (line == NULL)
			return (line);
